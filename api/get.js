export default async function handler(req, res) {
  try {
    ...
    // Fetch data without unnecessary headers
    const data0 = await fetchXMLWithRetry(apiUrl0);

    // Format XML response correctly (remove escaped newlines)
    const formattedXml = data0.replace(/\\n/g, "\n");
